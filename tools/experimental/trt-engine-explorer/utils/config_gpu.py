...
def install_pynvml():
    ...
    import subprocess
    try:
        subprocess.run("python3 -m pip install pynvml".split(), check=True)
    except subprocess.CalledProcessError:
        logging.warning("Failed to install pynvml.")
        logging.warning("Exiting: GPU clocks were not configured!")
        exit(1)
    ...
    def gpu_monitor(self):
        ...
        while not self.is_done:
            try:
                ...
                logging.warning("Could not read GPU state")
...
class GPUConfigurator():
    ...
    def _unlock_clocks(self):
        try:
            ...
            logging.warning("Unlocked device clocks.")
        except pynvml.nvml.NVMLError as e:
