...
def clamp_weights_onnx(
    ...
    for tensor in graph.nodes:
        ...
        if node_attr is not None and node_attr.values.dtype != "int64":
...
def clamp_weights_onnx_to_fp16_bounds(
    onnx_input_fpath: str, onnx_output_fpath: str, ignore_nodes: List = None
):
    upper_bound = 65504
    return clamp_weights_onnx(
        onnx_input_fpath, onnx_output_fpath, -upper_bound, upper_bound, ignore_nodes
    )


def clamp_weights_onnx_to_bf16_bounds(
    onnx_input_fpath: str, onnx_output_fpath: str, ignore_nodes: List = None
):
    upper_bound = 3.38953139e38
    ...
    for n in add_nodes:
        if n.o().op == "Pow":
            ...
            for i in add_inputs:
                ...
                new_cast = gs.Node(
                    op="Cast", inputs=[i], outputs=[identity_out], attrs={"to": 1}
                )
