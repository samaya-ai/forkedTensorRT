    ...
    class TorchModule(Module, GenerationMixin):
        ...
        def forward(self, input_ids, encoder_hidden_states, **kwargs):
            ...
            return Seq2SeqLMOutput(
                logits=logits,
                past_key_values=(
                    decoder_outputs.past_key_values if self.config.use_cache else None
                ),
            )
...
class T5DecoderTRTEngine(TRTEngineFile):
    ...
    def use_obey_precision_constraints(self):
        return self.network_metadata.precision.bf16
...
class T5DecoderConverter(ModelFileConverter):
    ...
    def torch_to_onnx(
        ...
        if network_metadata.precision.bf16:
            G_LOGGER.debug("Clamping FP16 weights for T5")
            move_t5_cast_op(output_fpath, output_fpath)
            clamp_weights_onnx_to_bf16_bounds(output_fpath, output_fpath)
...
class T5EncoderConverter(ModelFileConverter):
    ...
    def onnx_to_trt(
        ...
        if network_metadata.precision.bf16 and network_metadata.variant != "t5-small":
            network_metadata_cp_dct = network_metadata._asdict()
            del network_metadata_cp_dct["precision"]
            network_metadata = NetworkMetadata(
                **network_metadata_cp_dct, precision=Precision(bf16=False)
            )
    ...
    def torch_to_onnx(
        ...
        if network_metadata.precision.bf16:
            G_LOGGER.debug("Clamping BF16 weights for T5")
            move_t5_cast_op(output_fpath, output_fpath)
            clamp_weights_onnx_to_bf16_bounds(output_fpath, output_fpath)
