...
class T5Metadata(_T5Metadata, MetadataArgparseInteropMixin):
    ...
    def from_inference_args(args: argparse.Namespace):
        ...
        return base_metadata._replace(precision=Precision(bf16=True))
...
T5BenchmarkingArgs = namedtuple(
    "T5BenchmarkingArgs", ["input_seq_len", "output_seq_len"]
)

# trt has more benchmarking arguments
T5TRTBenchmarkingArgs = namedtuple(
    "T5TRTBenchmarkingArgs",
    [
        "input_seq_len",
        "output_seq_len",
...
class T5ModelTRTConfig(NNConfig):
    ...
    EMBEDDING_SIZE_PER_HEAD = {
        TARGET_MODELS[0]: ENCODER_HIDDEN_SIZE[TARGET_MODELS[0]]
        // NUMBER_OF_HEADS[TARGET_MODELS[0]],
        TARGET_MODELS[1]: ENCODER_HIDDEN_SIZE[TARGET_MODELS[1]]
        // NUMBER_OF_HEADS[TARGET_MODELS[1]],
        TARGET_MODELS[2]: ENCODER_HIDDEN_SIZE[TARGET_MODELS[2]]
        // NUMBER_OF_HEADS[TARGET_MODELS[2]],
        TARGET_MODELS[
            3
        ]: 128,  # For t5-3b and t5-11b, d_kv does not equal to d_model // num_heads in config, so needs hardcoded value for it
    }
    ...
    def get_input_dims(metadata) -> Dict:
        ...
        if metadata.other.kv_cache:
            ...
            for i in range(T5ModelTRTConfig.NUMBER_OF_DECODER_LAYERS[metadata.variant]):
                ...
                decoder_inputs_dict[f"past_key_values.{i}.decoder.key"] = (
                    self_attention_past_kv_dims
                )
                decoder_inputs_dict[f"past_key_values.{i}.decoder.value"] = (
                    self_attention_past_kv_dims
                )
                ...
                decoder_inputs_dict[f"past_key_values.{i}.encoder.key"] = (
                    cross_attention_past_kv_dims
                )
                decoder_inputs_dict[f"past_key_values.{i}.encoder.value"] = (
                    cross_attention_past_kv_dims
                )
    ...
    def get_output_dims(metadata) -> Dict:
        ...
        if metadata.other.kv_cache:
            ...
            for i in range(T5ModelTRTConfig.NUMBER_OF_DECODER_LAYERS[metadata.variant]):
                ...
                decoder_outputs_dict[f"present_key_values.{i}.decoder.key"] = (
                    self_attention_present_kv_dims
                )
                decoder_outputs_dict[f"present_key_values.{i}.decoder.value"] = (
                    self_attention_present_kv_dims
                )
                ...
                decoder_outputs_dict[f"present_key_values.{i}.encoder.key"] = (
                    cross_attention_present_kv_dims
                )
                decoder_outputs_dict[f"present_key_values.{i}.encoder.value"] = (
                    cross_attention_present_kv_dims
                )
